...
util.inherits(Kat, stream.Readable);
...
Kat.prototype._addStream = function(stream, sized, callback) {
  ...
  if (this.bytesRead > this.options.end || this._sized > this.options.end) {
    return callback();
  }
  ...
  // Set the encoding.
  if (this.options.encoding) stream.setEncoding(this.options.encoding);
  ...
  var data = {
    ...
    path: stream.path || this._totalFiles++,
    stream: stream,
    ...
    isFile: stream instanceof fs.ReadStream,
  ...
  function onend() {
    ...
    if (!self.readable) {
      ...
      } else if (self.options.autoEnd && self._openQueue.workers === 0) {
        self._end();
      } else {
...
Kat.prototype._addCurrentStream = function(data) {
  ...
  if (this._waiting) {
    // If `Kat#_read()` has already been called,
    // start reading from the stream right away.
    this._readStream(this._waiting);
...
Kat.prototype._readStream = function(size) {
  ...
  if (this.options.end < newPos) {
    size -= newPos - this.options.end;
  }
  ...
  if (data) {
    ...
    if (self.options.autoEnd && self.options.end < self.pos) {
      ...
      if (this._current.isFile) {
        fs.close(this._current.stream.fd);
      } else {
        this._current.onend();
      }
    }
 ...
 */
function isOldStyleStream(obj) {
  return typeof obj.read !== 'function' ||
    typeof obj._read !== 'function' ||
    typeof obj.push !== 'function' ||
    typeof obj.unshift !== 'function' ||
    typeof obj.wrap !== 'function';
}
